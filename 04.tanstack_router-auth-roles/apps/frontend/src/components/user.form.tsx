...
type CreateSubmit = { email: string; role: RoleValue; password: string };
type EditSubmit = { email: string; role: RoleValue };
...
export function UserForm<T>(props: Props<T>) {
  ...
  const [role, setRole] = useState<RoleValue>(props.initial.role ?? ROLE.USER);
  const [password, setPassword] = useState('');

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (isCreate) {
      await props.onSubmit({ email, role, password }); // password required
    } else {
      await props.onSubmit({ email, role }); // no password
    }
  };
