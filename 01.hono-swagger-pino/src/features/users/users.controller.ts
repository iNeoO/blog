...
const app = createLoggerFactory.createApp();

app.get('/:id', getUserRoute, (c) => {
...
app.get('/', getUsersRoute, (c) => {
  const users = getUsers();
  return c.json(users);
});

app.post('/', postUserRoute, zValidator('json', CreateUserSchema), async (c) => {
